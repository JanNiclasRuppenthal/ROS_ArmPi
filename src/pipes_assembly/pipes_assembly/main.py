...
def process_scenario(armpi, done_publisher, finish_publisher, pos_publisher, assemble_publisher, executor):
    ...
    while True:
        ...
        while armpi.get_assembly_queue().count() != armpi.get_number_of_robots():
    ...
    if armpi.get_ID() == armpi.get_assembly_queue().first():
        ...
        while (not armpi.get_assembly_queue().empty()): # all robots are done
            time.sleep(0.1)
        ...
        while (not armpi.get_ID() == armpi.get_assembly_queue().first()):
            time.sleep(0.1)
