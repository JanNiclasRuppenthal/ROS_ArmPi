...
def process_scenario(armpi, assembly_queue_publisher, holding_publisher, assembly_position_publisher, assembly_order_publisher, assembly_step_publisher, finish_publisher, executor):
    ...
    while True:
        ...
        while armpi.get_assembly_queue().count() != armpi.get_number_of_stationary_robots():
        ...
        armpi.set_assembly_queue_flag(False)
        dict_duplicates = armpi.get_assembly_queue().get_dict_with_duplicates()
        ...
        armpi.get_assembly_queue().reset()
        if put_down_object:
            put_down_grabbed_object(x, y, angle, rotation_direction, object_type)
            init_move()
            pipe_nr += 1
    ...
    if armpi.get_ID() == armpi.get_assembly_queue().first():
    
        order_queue = armpi.get_assembly_queue().get_queue()
    ...
    #TODO: I may need to reset some variable like assembly_queue and the pipe_nr
    armpi.get_assembly_queue().reset()
    pipe_nr = 0
...
def main():
    ...
    list_publisher_nodes = [
        create_assembly_queue_publisher_node(armpi),
    ...
    list_subscriber_nodes = [
        ...
        create_assembly_queue_subscriber_node(armpi),
